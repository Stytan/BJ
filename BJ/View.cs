...
namespace BJ
{
    public class View
    {
        ...
        public static void AskBid(Player aPlayer)
        {
            ...
            do
            {
                ...
                try
                {
                    ...
                    if (bid <= aPlayer.GetMoney() && bid >= MINBID)
                    {
                        aPlayer.SetBid(bid);
        ...
        //Показ стола
        public static void showTable(Table aTable, int tNum)
        {
            Console.Clear();
            Console.Write(" Table №" + tNum);
        ...
        public static bool Select(bool changeTable = false)
        {
            do
            {
                ...
                if (ch.Modifiers == ConsoleModifiers.Alt && changeTable)
                {
                    switch (ch.Key)
                    {
                        case ConsoleKey.F1:
        ...
        public static bool NextPlay()
        {
            Coord cur = AskXY;
            Console.SetCursorPosition(cur.X, cur.Y);
            Console.Write("You want to play more? (or change table Alt + F1-F10) (y/n) ");
            bool res = Select(true);
            Console.SetCursorPosition(cur.X, cur.Y);
            Console.Write("                                             ");
        ...
        public static void GameOver(int isWin)
        {
            ...
            switch (isWin)
            {
                case 0:
                    {
                        Console.Write("You lose all the money.");
                        Console.SetCursorPosition(cur.X - 4, cur.Y + 1);
                        Console.Write("Home will have to travel on foot.");
                        Console.ReadKey(true);
                        break;
                    }
                case 1:
                    {
                        Console.SetCursorPosition(cur.X - 8, cur.Y);
                        Console.Write("Congratulations you have cleaned the casino!");
                        Console.SetCursorPosition(cur.X - 19, cur.Y + 1);
                        Console.Write("At the output you expect responsible people, they will guide you ...");
                        Console.ReadKey(true);
                        break;
                    }
                case 2:
                    {
                        Console.Write("You take your money and go to rest...");
                        Console.SetCursorPosition(cur.X, cur.Y + 1);
                        Console.Write("Until we meet again!");
                        break;
                    }
            }
